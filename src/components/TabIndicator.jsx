...
export default class TabIndicator extends Component {
  ...
  setTabDots = () => {
    let tabArray = [], { totalTabs } = this.props

    for ( let i = 0; i < totalTabs; i++ ) {
      let el = ( <div id={this.props.tabIdPrefix + i} className="tabDot" key={i}>{
          i === 0
            ? this.tabSelected()
            : ''
        }</div> )
  ...
  tabSelected = () => {
    return ( <div className="tabSelected" style={{
        transform: `${ this.state.selectorPosition} scale(${ this.state.selectorScale}, ${ this.state.selectorScale })`
      }}/> )
  }
