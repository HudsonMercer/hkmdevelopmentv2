...
export default class TabIndicator extends Component{
  ...
  setTabDots = () => {
    ...
    for(let i = 0; i < totalTabs; i++ ){
      let el = (<div id={this.props.tabIdPrefix+i} className="tabDot" key={i}>{i === 0 ? this.tabSelected() : ''}</div>)
      tabArray.push(el)
    }
  ...
  setSelectorPosition = (index) => {
    let selectorPosition = '', rem = 2*index, px = 15*index
  ...
  tabSelected = () => {
    return(
      <div
        ...
        style={{transform: `${this.state.selectorPosition} scale(${this.state.selectorScale}, ${this.state.selectorScale})` }}
      />
    )
  }
